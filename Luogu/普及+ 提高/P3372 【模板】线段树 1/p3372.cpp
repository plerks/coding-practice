...
class SegTreeRangeAddRangeMax {
private:
    vector<long long> tree, lazy; // tree和lazy分别存储线段树和懒惰标记，tree[p]是相应区间的求和，lazy[p]是相应区间的lazy add量
    int n;
    ...
    long long range_sum(int L, int R, int s, int t, int p) { // [L, R]为查询区间, [s, t]为当前节点包含的区间, p为当前节点的编号
        ...
        if (!(R < s) && !(L > m)) sum += range_sum(L, R, s, m, p * 2);
        if (!(R < m + 1) && !(L > t)) sum += range_sum(L, R, m + 1, t, p * 2 + 1);
        return sum;
    }
    ...
    void range_add(int L, int R, int val, int s, int t, int p) { // [L, R]为查询区间, val为要add的值, [s, t]为当前节点包含的区间, p为当前节点的编号
        ...
        pushdown(s, t, p);
        int m = s + ((t - s) >> 1);
        if (!(R < s) && !(L > m)) range_add(L, R, val, s, m, p * 2);
        if (!(R < m + 1) && !(L > t)) range_add(L, R, val, m + 1, t, p * 2 + 1);
...
public:
    SegTreeRangeAddRangeMax(vector<int>& nums) {
        this->n = nums.size();
        ...
        叶节点，因此，线段树的高度为⌈logn⌉，最多有2^(⌈logn⌉ + 1) - 1个节点，又因为tree存储线段树节点从下标1开始，因此tree的size需要
        为2^(⌈logn⌉ + 1)。
        ...
        */
        ...
        build(nums, 0, n - 1, 1); // 线段树的根设置在tree[1]位置
    }

    long long range_sum(int L, int R) { return range_sum(L, R, 0, n - 1, 1); }
...
int main(int argc, char const *argv[]) {
    ...
    SegTreeRangeAddRangeMax segTree(v);
    while (m-- > 0) {
        int type, x, y, k;
        cin >> type;
        if (type == 1) {
            ...
            cin >> x >> y;
            cout << segTree.range_sum(x - 1, y - 1) << endl;
        }
    }
    return 0;
}
