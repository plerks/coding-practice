...
public:
    vector<int> nextGreaterElements_wrong(vector<int>& nums) {
        ...
        // 求左边最远的比nums[i]大的，注意单调栈解决不了这个问题，下面这么写是错误的，
        // [1,2,3,2,1]这个用例就能看出来，3把2出栈了，然后到最后的1时，st2.front()为nums[2]而非nums[1]
        //
        // 要解决这个问题似乎可以用值域线段树，每个叶子节点对应一个值，存储该值的最小索引。求左边最远的比nums[i]大的，即为求区间[nums[i] + 1, ..)内的最小值
        // 更简单的解法是先求前缀min，这样就能把nums变单调，然后二分查找第一个 < nums[i] 的位置，见AcWing4721. 排队
        vector<int> st2;
        vector<int> left(n, -1);
        for (int i = 0; i < n; i++) {
