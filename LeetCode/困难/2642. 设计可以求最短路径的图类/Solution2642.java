...
标签: 最短路径-增加边, Floyd算法, Dijkstra算法
*/
...
public class Solution2642 {
    ...
    // 粘贴到LeetCode上把这个方法改名为Graph
    public Solution2642(int n, int[][] edges) {
        this.n = n;
        this.D = new int[n][n];
    ...
    */
    ...
    /* 官方题解有个另外的用Dijkstra的解法，addEdge()时不重新计算最短路径，仅仅将边加入邻接表，然后shortestPath()查询时直接用
    Dijkstra算一遍node1到node2的最短距离。如果频繁shortedPath()查询而很少加边应该用Floyd，如果频繁加边而很少shortedPath()查询
    应该用Dijkstra
    */

    public static void main(String[] args) {
        Solution2642 solu = new Solution2642(4, new int[][]{{0, 2, 5}, {0, 1, 2}, {1, 2, 1}, {3, 0, 3}});
        System.out.println(solu.shortestPath(3, 2));
        System.out.println(solu.shortestPath(0, 3));
        solu.addEdge(new int[]{1,3,4});
        System.out.println(solu.shortestPath(0, 3));
    }
}
