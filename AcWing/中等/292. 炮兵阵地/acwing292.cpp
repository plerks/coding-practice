...
相关: LeetCode1349. 参加考试的最大学生数, AcWing291. 蒙德里安的梦想, LeetCode2617. 网格图中最少访问的格子数, 典型问题/滚动数组, 典型问题/位运算优先级的坑
...
int main(int argc, char const *argv[]) {
    ...
    因此滚动数组只需要2行即可。这里dpFull[0]没有前面的行dpFull[-1]，计算时dpFull[-1]等价于全0，刚好可以用滚动数组初始值为全0取代(相当于[-2, -1]也铺了个数组)，不用特判计算边界条件。
    ...
    注意i - k可能为负数，需要写成(i - k + w) % w，也可以单独处理dpFull的前w行，相当于先特判把初始的窗口算出来，这样就不涉及i - k可能为负数。

    这里由于窗口w大小为2，为2的幂次，x % 2^n等价于x & (2^n - 1)，所以可以直接用位运算求i前面的行，这里即使i - k可能为负用& (2^n - 1)也不会有问题，原因在于：
    0 >= i - k > -2^n，则0 <= -(i - k) < 2^n，则-(i - k)包含第n位往上一定全为0，则lowbit在[0, n - 1]位，(i - k)相对-(i - k)，lowbit之上的高位全相反，
    则i - k包含第n位往上一定全为1，则(i - k + 2^n)的效果是把i - k第n位往上全变为0，则(i - k + 2^n) % 2^n与(i - k) & (2^n - 1)相等。
    */
    for (int i = 0; i < m; i++) {
        for (int j : states) { // 第i行
