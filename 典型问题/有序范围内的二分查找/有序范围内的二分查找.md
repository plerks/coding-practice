...
首先，确定有序区间的回答是[是 ... 否]还是[否 ... 是]，对于平凡情况的searchFirst和searchLast，不是二分查找问题，直接给出结果。对于非平凡情况的searchFirst，统一转化为searchLast + 1，然后通过searchLast求解。

这两种有序区间情况对应的非平凡searchLast需求是`searchLast是`和`searchLast否`(`searchLast否`和`searchLast是`是平凡情况)。**把check()函数写成忠实地反映有序区间在对应位置上的回答**，然后为了让if(p)的p为真，要视情况在check()函数前面加上!。

这里一个最直观的操作方法是先写成`if (check() == true或false)`再简写为是否加!，

如果区间是[是 ... 否]，则回答是'是'时应该往右走left = mid + 1，于是应该写`if (check() == true)` <=> `if (check())`

如果区间是[否 ... 是]，则回答是'否'时应该往右走left = mid + 1，于是应该写`if (check() == false)` <=> `if (!check())`

### check最简洁的思考方式
需求为searchLast/searchFirst，如果是要searchFirst，先把判断条件转为searchLast的对应判断条件。

要搜索最后一个满足某某条件的位置，**直接将check()定义为这个条件**。然后看区间变化是`从是到否`还是`从否到是`。**如果是`从是到否`，则是`if (check())`；如果是`从否到是`，则是`if (!check())`**。

也即，check()忠实地写为判断条件，如果区间从否到是则需要在二分代码里if处写个取反。（注意，如果使用std::lower_bound并需要自定义比较器，如果需要取反的话，由于改不了std::lower_bound的代码，std::lower_bound里相当于固定是if(check())，这时就要在check()函数return时取反了，同样达到了把check结果反向的效果）

例如搜索最后一个`>=val`的位置，则check()定义为`nums[mid] >= val`，区间的变化为[是 ... 否]，则`if (check())`不需要加`!`。有个奇妙的点，searchLast如果是单纯的数值大小比较检查，首先，现在既然需要写二分，那肯定不是平凡情况。那么无论是搜索最后一个`<val, <=val, >val, >=val`（区间小->大，小->大，大->小，大->小），区间变化都一定为[是 ... 否]，所以一定都会是`if (check())`，不需要取反。区间从小到大，对于搜索第一个nums[mid] > val，区间是从否到是的，但是关键searchFirst要转为searchLast，条件会变为最后一个nums[mid] <= val，区间还是从是到否。再比如nums为111... 000，搜索最后一个为1的位置，也是从是到否。

对于条件的检查，例如是否能在t秒内跑完全程。才会出现区间为[否 ... 是]的情况，需要对check结果取反，即`if (!check())`。

最终**left - 1**是对应的last位置。
