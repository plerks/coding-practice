...
例如，如果区间是[是 ... 否]，则check()为真时应该进left = mid + 1，于是应该写`if (check() == true)` <=> `if (check())`

如果区间是[否 ... 是]，则check()为假时应该进left = mid + 1，于是应该写`if (check() == false)` <=> `if (!check())`
