...
此外要注意check()方法的返回值的问题，
```Java
if (check(nums, mid, target)) {
    left = mid + 1;
}
```
if中的逻辑表达式返回的值要与有序区间在mid位置上的回答一致。例如"LeetCode/困难/410. 分割数组的最大值/"中splitArray_implementation3()要二分搜索last '否'，则按那个check()的写法，if的逻辑表达式要写成`if (!check(nums, mid, k))`，当然也可以更改check()的实现，在check()内部反一下，这样就可以仍然公式化地写成`if (check(nums, mid, k))`。

总之，先确定有序区间的回答是[是 ... 否]还是[否 ... 是]，以要searchLast为例，这两种有序区间情况对应的searchLast需求一般会是`searchLast是`和`searchLast否`(`searchLast否`和`searchLast是`是平凡情况)，然后if(p)，这里p要忠实反映有序区间在对应位置上的回答，最终left - 1就会是对应的last位置。
