...
public:
    ...
    stack<int> st; // 暂存节点的栈
    vector<int> in_st; // 每个节点是否在栈中，在栈中代表被发现，但还未访问完毕
    vector<int> scc; int scc_id = 0; // 每个节点所在的SCC编号（从 1 开始），scc_id为编号数字。scc的个数 scc_cnt == scc_id
    vector<int> sz; // 强连通分量scc_id的大小
    vector<vector<int>> g;

    SCC(int n, const vector<pair<int, int>>& edges) {
        this->n = n;
        this->m = edges.size();
        dfn.resize(n);
        low.resize(n);
...
int main(int argc, char const *argv[]) {
    ...
    for (int i = 0; i < n; i++) {
        components[scc.scc[i]].push_back(i + 1); // 节点标号得加1回去
    }
    return 0;
}